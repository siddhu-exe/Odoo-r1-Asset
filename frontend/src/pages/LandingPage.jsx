...
function NumberCounter({ value, suffix = '', duration = 1.5 }) {
  ...
  return (
    <span ref={ref} className="font-bold text-4xl sm:text-5xl text-white">
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-lux-bg text-white overflow-hidden relative selection:bg-lux-cyan selection:text-lux-bg bg-grid-pattern">
      ...
      <div className="fixed right-6 top-1/2 -translate-y-1/2 z-50 flex flex-col gap-4 items-center hidden sm:flex">
        ...
        <div className="w-[1.5px] h-32 bg-lux-border relative">
          <motion.div 
            className="absolute top-0 w-full bg-lux-cyan shadow-[0_0_10px_#00d4ff]" 
            style={{ height: `${scrollProgress * 100}%` }}
          />
        </div>
        <div className="flex flex-col gap-2 mt-4">
          <div className={`w-1.5 h-1.5 rounded-full transition-colors ${scrollProgress < 0.25 ? 'bg-lux-cyan shadow-[0_0_6px_#00d4ff]' : 'bg-white/20'}`} />
          <div className={`w-1.5 h-1.5 rounded-full transition-colors ${scrollProgress >= 0.25 && scrollProgress < 0.65 ? 'bg-lux-cyan shadow-[0_0_6px_#00d4ff]' : 'bg-white/20'}`} />
          <div className={`w-1.5 h-1.5 rounded-full transition-colors ${scrollProgress >= 0.65 ? 'bg-lux-cyan shadow-[0_0_6px_#00d4ff]' : 'bg-white/20'}`} />
        </div>
      </div>
      ...
      <div className="fixed left-6 bottom-6 z-50">
        <div className="relative">
          <button 
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-full glass-lux text-xs font-semibold text-text-secondary hover:text-white border border-lux-border hover:border-lux-cyan/40 transition-all duration-300"
          >
            <Globe size={12} className="text-lux-cyan" />
            <span>{lang}</span>
          ...
          <AnimatePresence>
            {langOpen && (
              <motion.div 
                ...
                className="absolute bottom-full left-0 mb-2 w-24 rounded-lg glass-lux p-1 shadow-2xl border border-lux-border text-xs flex flex-col gap-1 overflow-hidden"
              >
                {['EN', 'AE', 'DE', 'FR'].map((l) => (
                  <button
                    key={l}
                    onClick={() => { setLang(l); setLangOpen(false); }}
                    className={`w-full py-1.5 px-2 rounded text-left hover:bg-white/10 transition-colors ${lang === l ? 'text-lux-cyan font-bold bg-white/5' : 'text-text-secondary'}`}
                  >
                    {l === 'AE' ? 'العربية' : l}
                  </button>
                ))}
      ...
      <div className="relative h-screen w-full flex items-center justify-center overflow-hidden bg-[#030303]">
        ...
        <motion.div 
          ...
          initial="initial"
          animate={maskState}
          className="absolute overflow-hidden bg-lux-bg flex items-center justify-center shadow-2xl shadow-lux-purple/10 border border-lux-border/50"
        >
          {/* Interactive R3F Scene */}
          <div className="absolute inset-0 z-0">
            <PortalScene scrollProgress={scrollProgress} />
          </div>

          {/* Visual gradient overlays to darken base for text visibility */}
          <div className="absolute inset-0 bg-gradient-to-t from-lux-bg via-lux-bg/40 to-transparent z-10 pointer-events-none" />
          <div className="absolute inset-0 bg-gradient-to-r from-lux-bg/85 via-transparent to-lux-bg/85 z-10 pointer-events-none" />

          {/* Hero Content */}
          <div className="relative z-20 max-w-5xl mx-auto px-6 text-center flex flex-col items-center">
            
            {/* Soft floating glow indicators */}
            <div className="absolute -top-32 w-96 h-96 bg-lux-purple/10 rounded-full blur-3xl pointer-events-none" />
            <div className="absolute -bottom-32 w-96 h-96 bg-lux-cyan/5 rounded-full blur-3xl pointer-events-none" />
            ...
            <motion.div 
              ...
              className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10 text-lux-sky text-xs font-semibold uppercase tracking-wider mb-6 backdrop-blur-md"
            >
              <span className="w-1.5 h-1.5 rounded-full bg-lux-cyan animate-pulse shadow-[0_0_6px_#00d4ff]" />
            ...
            >
              <Link 
                to="/login"
                className="btn-primary w-full sm:w-auto px-8 py-4 bg-gradient-to-r from-lux-purple to-lux-cyan text-white font-bold rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-lux-purple/30 hover:shadow-lux-cyan/40 hover:scale-105 active:scale-95 transition-all duration-300 group"
              >
              ...
              <a 
                href="#features"
                className="w-full sm:w-auto px-8 py-4 bg-white/5 border border-white/10 hover:border-lux-cyan/40 font-semibold rounded-xl text-white hover:bg-white/10 hover:scale-105 active:scale-95 transition-all duration-300 flex items-center justify-center backdrop-blur-md"
              >
                Explore Features
              </a>
            </motion.div>
      ...
      <section id="features" className="py-24 relative z-20 max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="absolute top-20 left-10 w-64 h-64 bg-lux-cyan/5 rounded-full blur-3xl pointer-events-none -z-10" />
        <div className="absolute bottom-20 right-10 w-96 h-96 bg-lux-purple/5 rounded-full blur-3xl pointer-events-none -z-10" />

        <div className="text-center max-w-3xl mx-auto mb-16">
          <h2 className="text-xs font-semibold text-lux-cyan uppercase tracking-widest mb-3">Architected For Scalability</h2>
          ...
          <div className="w-16 h-[2px] bg-gradient-to-r from-lux-purple to-lux-cyan mx-auto mt-6" />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
          ...
          >
            <CornerBorders className="glass-lux-card p-8 h-full glow-hover rounded-xl flex flex-col justify-between">
              <div>
                <div className="w-12 h-12 rounded-lg bg-lux-cyan/10 border border-lux-cyan/20 flex items-center justify-center text-lux-cyan mb-6">
                  <Database size={24} />
                </div>
                <h3 className="text-xl font-bold text-white mb-3">Centralized Asset Directory</h3>
              ...
              <div className="border-t border-white/5 pt-4 flex items-center justify-between text-xs text-lux-cyan font-semibold">
          ...
          >
            <CornerBorders className="glass-lux-card p-8 h-full glow-hover rounded-xl flex flex-col justify-between">
              <div>
                <div className="w-12 h-12 rounded-lg bg-lux-sky/10 border border-lux-sky/20 flex items-center justify-center text-lux-sky mb-6">
                  <Workflow size={24} />
                </div>
                <h3 className="text-xl font-bold text-white mb-3">Conflict-Free Bookings</h3>
                <p className="text-text-secondary text-sm leading-relaxed mb-6">
              ...
              <div className="border-t border-white/5 pt-4 flex items-center justify-between text-xs text-lux-sky font-semibold">
          ...
          >
            <CornerBorders className="glass-lux-card p-8 h-full glow-hover rounded-xl flex flex-col justify-between">
              <div>
                <div className="w-12 h-12 rounded-lg bg-lux-purple/10 border border-lux-purple/20 flex items-center justify-center text-lux-purple mb-6">
                  <Layers size={24} />
                </div>
                <h3 className="text-xl font-bold text-white mb-3">Kanban Maintenance Board</h3>
                <p className="text-text-secondary text-sm leading-relaxed mb-6">
              ...
              <div className="border-t border-white/5 pt-4 flex items-center justify-between text-xs text-lux-purple font-semibold">
          ...
          >
            <CornerBorders className="glass-lux-card p-8 h-full glow-hover rounded-xl flex flex-col justify-between">
              <div>
                <div className="w-12 h-12 rounded-lg bg-lux-magenta/10 border border-lux-magenta/20 flex items-center justify-center text-lux-magenta mb-6">
                  <BarChart3 size={24} />
                </div>
                <h3 className="text-xl font-bold text-white mb-3">Audits & Reports</h3>
                <p className="text-text-secondary text-sm leading-relaxed mb-6">
              ...
              <div className="border-t border-white/5 pt-4 flex items-center justify-between text-xs text-lux-magenta font-semibold">
      ...
      <section className="py-20 relative z-20 max-w-7xl mx-auto px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-16">
          <h2 className="text-xs font-semibold text-lux-purple uppercase tracking-widest mb-3">Seamless Control</h2>
          <p className="text-3xl sm:text-5xl font-extrabold tracking-tight text-white">
        ...
        <motion.div 
          ...
          className="relative rounded-2xl glass-lux border border-lux-border p-4 md:p-6 overflow-hidden shadow-2xl shadow-lux-purple/5"
        >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-3 hidden lg:flex flex-col gap-2.5 border-r border-white/5 pr-6 text-sm">
              <div className="p-3 bg-white/5 text-white font-semibold rounded-lg flex items-center gap-2 border border-white/5">
                <div className="w-1.5 h-1.5 rounded-full bg-lux-cyan" />
              ...
              {['Asset Registry', 'Resource Scheduler', 'Kanban Board', 'Audit Cycles', 'Analytics Reports', 'Department Settings'].map((menu, i) => (
                <div key={i} className="p-2.5 hover:bg-white/5 text-text-secondary hover:text-white rounded-lg flex items-center gap-2 cursor-pointer transition-colors text-xs">
            ...
            <div className="lg:col-span-9 flex flex-col gap-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {[
                  { label: 'Hardware Stock', value: '4,821 Units', change: '+12% MoM', color: 'text-lux-cyan' },
                  { label: 'Active Custodians', value: '1,498 Staff', change: '+3% MoM', color: 'text-lux-sky' },
                  { label: 'Pending Repairs', value: '14 Tickets', change: '-28% Weekly', color: 'text-lux-purple' },
                  { label: 'Compliance Level', value: '98.4%', change: 'Grade A Audit', color: 'text-lux-magenta' },
                ].map((stat, i) => (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="glass-lux-card rounded-xl overflow-hidden border border-white/5 flex flex-col">
                  <div className="relative h-44 overflow-hidden bg-black/40">
                    ...
                    />
                    <div className="absolute top-3 right-3 px-2 py-0.5 bg-lux-cyan/80 text-background text-[10px] font-bold uppercase rounded-md tracking-wider">
                      Available
                    </div>
                  </div>
                  <div className="p-4 flex-1 flex flex-col justify-between">
                    <div>
                      <span className="text-[9px] text-lux-cyan font-mono uppercase tracking-widest">ID: ASSET-88219</span>
                      <h4 className="text-sm font-bold text-white mt-1">Apple Mac Studio (M2 Ultra, 64GB)</h4>
                    ...
                    <div className="flex items-center justify-between text-[10px] text-text-secondary/50 mt-4 border-t border-white/5 pt-3">
                ...
                <div className="glass-lux-card rounded-xl overflow-hidden border border-white/5 flex flex-col">
                  <div className="relative h-44 overflow-hidden bg-black/40">
                    <img 
                      ...
                      className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" 
                    />
                    <div className="absolute top-3 right-3 px-2 py-0.5 bg-lux-purple/80 text-white text-[10px] font-bold uppercase rounded-md tracking-wider">
                      Allocated
                    </div>
                  </div>
                  <div className="p-4 flex-1 flex flex-col justify-between">
                    <div>
                      <span className="text-[9px] text-lux-purple font-mono uppercase tracking-widest">ID: ASSET-90114</span>
                      <h4 className="text-sm font-bold text-white mt-1">Dell PowerEdge R760 Rack Server</h4>
